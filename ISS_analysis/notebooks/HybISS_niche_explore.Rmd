...
ASM_HGF_data <- cellsPhen.niches%>%filter(cell_type=="ASM" & marker=="HGF")
ASM_HGFdata.shuffled <- ASM_HGF_data[sample(1:nrow(ASM_HGF_data)), ]
ggplot(data = ASM_HGF_data,aes(x=a4,y=value))+
  geom_point()+
  stat_cor(method="spearman",cor.coef.name="rho",label.x=0.2)+
  stat_cor(method="pearson",cor.coef.name="R",label.y=2.7,label.x=0.2)+
  xlab("vessel niche weight")+ ylab(paste0("HGF expression"))+
  ggtitle("HGF expr in ASM VS vessel niche")

ggplot(data = ASM_HGFdata.shuffled,aes(x=a4,y=value))+
  geom_point()+
  stat_cor(method="spearman",cor.coef.name="rho",label.x=0.2)+
  stat_cor(method="pearson",cor.coef.name="R",label.y=2.7,label.x=0.2)+
  xlab("vessel niche weight")+ ylab(paste0("HGF expression"))+
  ggtitle("HGF expr in ASM VS vessel niche (shuffled)")
## Contigency table
table(ASM_HGF_data%>%mutate(vessel=ifelse(a4>=0.2,1,0))%>%dplyr::select(c(vessel,value)))
## Linear regression over HGF mRNA counts (3 levels)
model1 <- lm( a4 ~ as.factor(value), data = ASM_HGF_data)
summary(model1)
