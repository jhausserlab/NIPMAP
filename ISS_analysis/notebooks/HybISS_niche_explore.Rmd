...
if (file.exists(paste(ROOT_DATA_PATH,"cellData_",ImageIDs,".csv") %>% str_replace_all(" ", ""))){
  ...
  cellMarkers <- read.csv("../My_cell_positions_data/ISS_dataset/Cell_type_deep_clusters/PCW13/S1T1_mALL_complex_celltypes_formatted.csv")
  ...
  colnames(cellMarkers_new) = c("patient_id", "cell_id", "X", "Y", "cell_type", "complex_celltype", all_of(MARKERS))
...
markers <- cell_markers %>% 
  select(-c(patient_id, cell_id, X, Y, cell_type, complex_celltype)) %>% 
  ZscoreScale() %>% 
  t() %>% 
  as.data.frame()
...
markersPerCell <- cell_markers%>%pivot_longer(cols = all_of(MARKERS), names_to = "marker", values_to = "value")
 ...
 if (file.exists("../data/MarkersCellsTMENs_filtered.rds")){
  ...
  MarkersCellsTMENS <- left_join(markersPerCell %>% rename(site_id=cell_id), tmens_per_cell, by=c("patient_id","site_id")) %>%
...
# plot_heatmap_CT(cMf,coreIntf2)
  ...
  CM_TMENs_ct <- as_tibble(cMf,rownames=NA)%>%
    ...
    pivot_longer(cols=coreIntf2,names_to = "region",values_to="corr_val")%>%
...
```{r, fig.height=6, fig.width=8, echo=FALSE, warning=FALSE, message=FALSE}
  ...
  # write_csv(cMf%>%as_tibble(rownames=NA)%>%rownames_to_column(var="cell_marker"), "./CM_corr_tmensREDUCED_thresh0.3.csv")
  
  cMf_fig <- cMf
  colnames(cMf_fig) <- str_replace_all(colnames(cMf_fig), c("a2a3"="mesenchymalxsmooth muscle",
                                                           "a1a3" = "epithelial x smooth muscle",
                                                           "a1a2"= "epithelial x mesenchymal",
                                                           "a1a4" = "epithelial x arterial",
                                                           "a2a4" ="mesenchymal x arterial",
                                                           ...
                                                           "a3a5" = "smooth muscle x ductal/alveolar", 
                                                           "a4a5" = "arterial x ductal/alveolar"))
  colnames(cMf_fig) <- str_replace_all(colnames(cMf_fig),c("a1" = "epithelial","a2" = "mesenchymal","a3" = "smooth muscle","a4" = "arterial", "a5"= "ductal/alveolar"))
  
  ct_df <-as_tibble(cMf_fig,rownames=NA)%>%rownames_to_column(var="names")%>%separate(names,into=c("cell_type","marker"),sep=";")
  annotation <- data.frame(as.factor(ct_df$cell_type)) %>% `colnames<-`("Cell Type")
  row.names(annotation) <- rownames(cMf_fig)
  
  pheatmaps.obj <- pheatmap::pheatmap(cMf_fig%>%t(), 
                     main = paste("qThresh=",qThresh, "corThresh=",corThresh), 
                     cluster_cols = FALSE, 
                     # cluster_rows = FALSE,
                     # color = viridis(256), 
                     border_color = NA, 
...
ggsave("../output/SS_tmens.pdf",height=10,width=4)
```
