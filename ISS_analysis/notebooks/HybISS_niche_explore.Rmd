...
#### Visualization of niche weights VS phenotype expression of cells


```{r pltNichesGenes, echo=TRUE}

### Visualize niche- phenotype associations: 
# JAG1+ arterial cells in arterial niche
cellType <-"arterial"
Marker <- "JAG1"
niche <- "a4"
plot1 <- ggplot(data=cellsPhen.niches%>%filter(cell_type==cellType & marker==Marker),aes_string(x=niche,y="value"))+
      geom_point(alpha=.1)+
...
plot1
boxplot(x=cellsPhen.niches%>%filter(cell_type==cellType & marker==Marker)%>%pull(value))

# "FZD1+ Dendrtitic cells in arterial niche
cellType <-"Dendritic"
Marker <- "FZD1"
niche <- "a4"
plot2 <- ggplot(data=cellsPhen.niches%>%filter(cell_type==cellType & marker==Marker),aes_string(x=niche,y="value"))+
      geom_point(alpha=.1)+
      stat_cor(method="spearman",cor.coef.name="rho",label.x=0.1)+
      stat_cor(method="pearson",cor.coef.name="R",label.y=4.0,label.x=0.1)+
      xlab(paste0(niche,collapse=" x "))+ ylab(paste0(Marker," expression"))+
      ggtitle(paste0(Marker,"⁺ ",cellType," VS ",niche))
plot2
#boxplot(apply(cell_markers%>%dplyr::select(-c(SampleID, cellLabelInImage, X, Y, cell_type, complex_celltype)),2,mean))

## Distribution of mean mRNA counts(raw, not Z-score normalized)
CellsmRNA<-cell_markers%>%dplyr::select(-c(SampleID, cellLabelInImage, X, Y, cell_type, complex_celltype))%>%as.matrix()
mRNA.means <- data.frame(meanRNA_counts = apply(cell_markers%>%dplyr::select(-c(SampleID, cellLabelInImage, X, Y, cell_type, complex_celltype)),2,mean))
ggplot(data=mRNA.means,aes(x=meanRNA_counts))+
  geom_density()

# Check the cells that have few mRNA expressed ==> fitler out ?
# CellsmRNA[which(rowSums(CellsmRNA)<=10),]

## Distribution of mean value of mRNA counts after Z-score normalization 
mRNA.meansNorm <- data.frame(meanRNA_counts = apply(cell_markers%>%dplyr::select(-c(SampleID, cellLabelInImage, X, Y, cell_type, complex_celltype)),2, mean))
ggplot(data=mRNA.meansNorm,aes(x=meanRNA_counts)) +
  geom_density()


# ggplot(data=data.frame(meanRNA_counts =apply(cell_markers%>%mutate_at(.vars = MARKERS,scale)%>%dplyr::select(-c(SampleID, cellLabelInImage, X, Y, cell_type, complex_celltype))%>%as.matrix(),2,mean)),aes(x=meanRNA_counts))+
#   geom_density()
...
#### Analysis of correlation statistics

```{r}
correlationMatrix <- read_csv("./NiPhcorrelations.csv")
CorpvalueMatrix <- read_csv("./NiPhpvalue.csv")
CorqvalueMatrix <- read_csv("./NiPhqvalue.csv")

hist(as.vector(as.matrix(correlationMatrix)))
hist(as.vector(as.matrix(log(CorpvalueMatrix))))
hist(as.vector(as.matrix(log(CorqvalueMatrix))))
...
## Niche-phenotype mapping on 800x 800 images
...
```{r}
# data set of cells with their niches weights computed from 800 x 800 image: cells_niches_xs.json file
#TODO repeat histograms analysis on  cells niches weights from 800x800 image
#TODO repeat scatter plots of JAG1 arterial VS arterial niche and FZD1 Dendritic in arterial niche on cells niches weights from 800x 800 image SEE histCorr, pltNichesGenes and CM chunks to repeat
file5 = paste("/scratch/ziqi.kang/HIFI_TMENs_analysis/My_cell_positions_data/ISS_dataset/Region_test_6500/niche5_radius25_patientid[1]/cells_niches.json") # sites centered on cells and niches weights
cells_niches_xs <- fromJSON(file=file5)
```

```{r, include=FALSE}
#######---- Open .json files ----#######

file6 = paste("/scratch/ziqi.kang/HIFI_TMENs_analysis/My_cell_positions_data/ISS_dataset/Region_test_6500/niche5_radius25_patientid[1]/pca_sites.json") %>% str_replace_all(" ", "") # pca object on sites elements 
file7 = paste("/scratch/ziqi.kang/HIFI_TMENs_analysis/My_cell_positions_data/ISS_dataset/Region_test_6500/niche5_radius25_patientid[1]/AA_sites.json") %>% str_replace_all(" ", "") # archetype Analysis object based on sites cell abundance
file8 = paste("/scratch/ziqi.kang/HIFI_TMENs_analysis/My_cell_positions_data/ISS_dataset/Region_test_6500/niche5_radius25_patientid[1]/ca_sites.json") %>% str_replace_all(" ", "") # cell abundance of randomly generated sites

pca_sites_xs <- fromJSON(file=file6)
AA_sites_xs <- fromJSON(file=file7)
ca_sites_xs <- fromJSON(file=file8)
...
# prepare tmens_per_cell table
tmens_per_cell <- cells_niches_xs$cells_niches
tmens_per_cell <- do.call(cbind, tmens_per_cell) %>% 
  as.data.frame() %>% 
  `colnames<-`(c(core_niche, "patient_id", "site_id",
                              "cell_type_site", "TOT_cell_dens"))
...
## Distribution of mean value of mRNA counts after Z-score normalization 
mRNA.meansNorm <- data.frame(meanRNA_counts = apply(cell_markersNorm%>%dplyr::select(-c(SampleID, cell_id, X, Y, cell_type, complex_celltype)),2,mean))
ggplot(data=mRNA.meansNorm,aes(x=meanRNA_counts))+
  geom_density()
...
cMf <- CM
# Plot heatmaps of niches-phenotypes associations
plot_heatmap_CT(CM.mat=cMf,coreIntf2,"../output/CMbyCells_xs.pdf")
...
hist(as.vector(as.matrix(log(correlationMatrix))))
hist(as.vector(as.matrix(log(CorpvalueMatrix))))
hist(as.vector(as.matrix(log(CorqvalueMatrix))))
...
### Display and save table in pdf
 ...
 grid.newpage()
 grid.draw(g1)
 ggsave("../output/tab_tmens_xs.pdf",g1,width=7,height=6)
```
