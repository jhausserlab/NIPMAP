...
def get_segmentation_matrix(data, cell_types, pca_obj, archetype_obj, color_fun, h=800, w=800, counting_type='abs', radius=100, granularity=25):
    ...
    for grid_point in combinations:
        ...
        _, alfa = archetype_obj.transform(new_pc[:, :archetype_obj.alfa.shape[0]-1]) # FIXME archtype_obj.alfa.shape[0]
        # print(archetype_obj.alfa.shape)
...
def plot_cells_positions(data, cell_types, segment_image=False, segmentation_type='hard', counting_type='abs', color_vector=[[255, 0, 223],[255, 0, 0],[70, 203, 236],[0, 0, 0]], h=800, w=800, granularity=25, radius=25, pca_obj=None, AA_obj=None, to_plot='all',path_fig=None):
    '''
    ...
    for (name, group), col in zip(groups, colors):
        if to_plot == 'all' or name in to_plot:
            plt.scatter(group['x'], group['y'], marker="o", s=1, label=name, c=cells_cols[name])
...
def plot_cells_markers_tmens(patient_id,cell_types,path_data, data_markers_path,cell_type, marker,symbols,col=None,segment_image=False,
   ...
   # data = data.loc[data["cell_type"] == cell_type]
    ...
    if segment_image is True:
        if pca_obj is None or AA_obj is None:
                raise ValueError("To segment the image pca and archetypes objects are needed")
        color_vector =  np.array([[255, 0, 223],[0, 0, 255],[70, 203, 236],[255, 0, 0], [0,0,0]])#np.array([[255, 0, 0], [0, 153, 51], [0, 0, 255], [255, 255, 0]])
        if segmentation_type == 'hard': #color pixel by 1 of the colors defining TMENs (discrete)
                color_fun = partial(alfa2color, color_vector)
        ...
        df_markers.rename(columns = {"cell_id":"label"},inplace=True)
...
def plot_all_tumors_cell_positions(patient_ids, cell_types, segment_image=False, segmentation_type='hard', radius=100, counting_type="gaussian", h=800, w=800, 
                                   color_vector=[[255, 0, 223],[255, 0, 0],[70, 203, 236],[0, 0, 0]], 
    ...
    for i, patientID in enumerate(patient_ids):
        ...
        if segment_image is True:
            if pca_obj is None or AA_obj is None:
                raise ValueError("To segment the image pca and archetypes objects are needed")
            # color_vector = np.array([[255, 0, 223],[255,0,0],[70,203,236],[0,0,0],[255, 255, 0],[0, 153, 51], [0, 0, 255]])#np.array([[255, 0, 0], [0, 153, 51], [0, 0, 255], [255, 255, 0]])
            if segmentation_type == 'hard': #color pixel by 1 of the colors defining TMENs (discrete)
                color_fun = partial(alfa2color, color_vector)
            ...
            try:
                ...
                h = 500
                w = 500
            z = get_segmentation_matrix(data, cell_types, pca_obj, AA_obj, color_fun, counting_type = counting_type, granularity=granularity, h=h, w=w,
                                        radius=radius)
            plt.imshow(z, origin='lower')
    ...
    plt.legend(scatterpoints=1, fontsize=10, loc='center right', bbox_to_anchor=(1.25, 0.5))
#        for (name, group), col in zip(groups, colors):
#            if to_plot == 'all' or name in to_plot:
...
def archetypes_bar_plot(cell_number_archetypes, cell_types, colors, y_axis='count', radius=100, path_fig=None):
    ...
    #print()
    nbs = [1,2,3,4,5,6,7,8] # FIXME controls how many bars could be add
    #3-->1, 1-->4, 2-->2 , 4-->3
    ...
    plt.savefig(path_fig,format="svg",bbox_inches='tight')
    plt.show()
