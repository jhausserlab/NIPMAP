...
def generate_abundance_matrix(cell_types, patient_ids, n_site, radius, method, snr=1, root="../../output/", image_x_size=800, image_y_size=800, 
                              random_seed=False,center_sites_cells=False):
    return [CellAbundance(p, n_site, radius, cell_types, method=method, snr=snr, root=root, image_x_size=image_x_size, image_y_size=image_y_size, random_seed=random_seed,center_sites_cells=center_sites_cells) for
            p in patient_ids]
...
class CellAbundance:
    def __init__(self, patient_id, n_sites,
                 ...
                 method='abs', norm_method=None, snr=1, random_seed=False, image_x_size=800, image_y_size=800,center_sites_cells=False):
        if random_seed:
    ...
    def calculate_site_groups(self):
        ...
        # Center sites on cells or generate randomly(uniform) 100 sites per imagpe    
        if self.center_sites_cells == False:
            #x_centers = np.random.uniform(low=np.min(x) + radius, high=np.max(x) - radius,size=self.n_sites)
            #y_centers = np.random.uniform(low=np.min(y) + radius, high=np.max(y) - radius,size=self.n_sites)
