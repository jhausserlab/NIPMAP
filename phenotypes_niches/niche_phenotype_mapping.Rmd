...
#FIXME computing interfaces: use combn to allow for more interfaces and more ncihes (see Ziqi's suggestion code)
...
plot_roc_niches_cells(CT = CTroc,clustFiles = fileExt,clustIDs = clustCells, 
                      nichesNames = niches,markers = FuncMarkers,
                      MarkersCells.niches = MarkersCellsTMENs.byNiche)
...
coreIntf2<-c("arch1","arch2","arch3","arch4","a1a2","a1a3","a1a4","a2a3","a2a4","a3a4")#,"a1a2a3","a1a2a4","a1a3a4","a2a3a4","a1a2a3a4")
...
cMf <-correlation_niches_CM(markersCells.niches=MarkersCellsTMENS2,Markers=FuncMarkers,corrMeth="spearman",coreIntf=coreIntf2)

write_csv(cMf%>%as_tibble(rownames=NA)%>%rownames_to_column(var="cell_marker"), "./CM_corr_niches_thresh0.3.csv")
figWidth <- nrow(cMf) * 30/267

Krt.filt <- names(which(cMf[rownames(cMf)[which(grepl("Keratin6",rownames(cMf),fixed=TRUE))],"arch3"]>0.3))
BCat.filt <- names(which(cMf[rownames(cMf)[which(grepl("Beta catenin",rownames(cMf),fixed=TRUE))],"arch3"]>0.2))
...
#TODO make function to create table of cell phenotypes
#TODO add different orders of interfaces
#FIXME consistency in names of archetypes: arch1, arch2,3,4, or a1,a2,a3 etc...
#FIXME use another input dataset: no .rds file from another path file ! 
...
#get_cell_type_enriched_arch(cellAb.df=archsSitesCellAb,archetype="archetype4",thresh=0.99)

archs.CT <- get_CT_enriched_all_archs(archetypesSitesKeren,cellAbSites,thresh = 0.99)%>%group_by(niche)%>%filter(!(cell_type %in% c("DC / Mono", "CD3-T", "Mono / Neu", "Other immune","Unidentified")))%>%mutate(cell_type = paste(unique(cell_type),collapse="\n"))%>%distinct(niche,cell_type)
