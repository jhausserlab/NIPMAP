...
CellsFunctMarkers.df <- cell_markers[which(cell_markers$cell_type %in% c("DC")),append(append(FuncMarkers,"cell_type"),"cell_id")]#%>%mutate(id = paste0(cell_type,"_",cell_id))%>%column_to_rownames(var="id")#%>%group_by(cell_type) c("DC", "Keratin-positive tumor", "NK", "Neutrophils", "Tregs")
rownames(CellsFunctMarkers.df) <- make.names(paste0(CellsFunctMarkers.df$cell_type,"_",CellsFunctMarkers.df$cell_id),unique=TRUE)
...
cm.mat <- CellsFunctMarkers.df%>%dplyr::select(-c(cell_type,cell_id))%>%ZscoreScale()%>%t()
...
hclust_cellTypes <- function(){
  ...
  CellsFunctMarkers.df <- cell_markers[which(cell_markers$cell_type %in% c("DC", "NK", "Neutrophils", "Tregs","Macrophages")),append(append(all_of(FuncMarkers),"cell_type"),"cell_id")]# "Keratin-positive tumor"
  
  rownames(CellsFunctMarkers.df) <- make.names(paste0(CellsFunctMarkers.df$cell_type,"_",CellsFunctMarkers.df$cell_id),unique=TRUE)
  #print(rownames(CellsFunctMarkers.df))
  cells.markers <- group_split(CellsFunctMarkers.df%>%group_by(cell_type))
  #print(cells.markers)
  ...
  list_heatmaps <-lapply(cells.markers, FUN = function(x) {
    ...
    cm.mat <- x%>%dplyr::select(-c(cell_type,cell_id))%>%ZscoreScale()%>%t()
    print(dim(cm.mat))
    CM.dist <- dist(cm.mat,method="euclidian")
    Cells.hclust <- hclust(CM.dist,method = "ward.D")
    #print(table(cutree(Cells.hclust, k = 10)))
    sub_grp <-  data.frame(cell_id=seq(1,nrow(x),1),cluster=cutree(Cells.hclust, k = 10),row.names=NULL)
    #cm.mat[,'cluster'] <- sub_grp
    
    clustMarkers.zscore <- cm.mat %>%as_tibble()%>%mutate(cell_id=seq(1,nrow(x),1))%>%#as_tibble(rownames=NA)%>%rownames_to_column(var="cell_id")%>%
      left_join(.,sub_grp, by="cell_id")%>%
      pivot_longer(cols = FuncMarkers, names_to="marker",values_to = "value")%>%
      group_by(cluster,marker)%>%
      mutate(meanMClust = mean(value))%>%
...
## TODO 
### Remove markers that have a weak association to the TMENs
### OR keep all markers in correlation matrix for each cell to make it easy to compare 
...
tmens_cells2 <- read.csv("../data_BB/archetypes_sites_centered_cells_gaussian2.csv")%>%filter(cell_type_site!="Unidentified")
...
## Marker-space association

To do so, we'll fit a dirichlet (modified) distribution onto the intensity of a marker of a cell type.
...
```{r GradDesc, eval=FALSE,echo=TRUE}
...
x_cm <- get_tmens_coord_marker("B","CD20",eps)
#x_cm2 <- get_tmens_coord_marker("CD4-T","PD1")
estim_params <- optim(par = init, fn = func,data = x_cm,method = "L-BFGS-B",lower = c(-100,-100,-100,-100,0),upper = c(rep(100,NB_TMENs),Inf),
                       control = list(trace = 3,
                              maxit = 1000))
#method = "L-BFGS-B",lower = rep(0,5),upper = c(rep(100,NB_TMENs),Inf),

#params2 <- optim(par= init, fn= func,data = x_cm2,method = "L-BFGS-B",lower = c(-100,-100,-100,-100,0),upper=c(100,100,100,100,Inf))

```


```{r plots2, eval=FALSE,echo=TRUE}

params_PD1_CD8T <-read.csv("./GradDesc_CM_tmens/fittedParamsGammaKm_PD1_CD8T.csv")
p1 <- params_PD1_CD8T%>%pull(x)
params_CD20_B<- read.csv("./GradDesc_CM_tmens/fittedParamsGammaKm_CD20_B.csv")
p2 <- params_CD20_B%>%pull(x)
x_cm <- get_tmens_coord_marker("B","CD20",10^-4)
f1 <- function(pars,x_cm){
  res <-(pars[NB_TMENs+1]* apply(x_cm[c("arch1","arch2","arch3","arch4")],1,function (x) prod(diag(outer(x,pars[1:NB_TMENs],FUN="^")))))
  return(res)
} 
x_cm_fit <- x_cm %>%mutate(fitted_val=f1(p1,x_cm))
#plot(x_cm$arch1,x_cm$value)
ggplot(data = x_cm_fit)+
  geom_point(aes(x=arch1,y=value))+
  geom_line(aes(x=arch1,fitted_val,color="red"))

```


```{r scaleUp,eval=FALSE,echo=TRUE}

#m2 <- c("Pan.Keratin","Keratin6","Keratin17","p53","EGFR","Beta.catenin","dsDNA","H3K9ac","H3K27me3","phospho.S6","Ki67","CD45","CD3","CD8","CD4","FoxP3","CD20","CD45RO","CD56","CD16","CD138","CD11b","CD11c","CD68","CD63","MPO","HLA.DR","HLA_Class_1","CD209","Lag3","PD1","PD.L1","IDO","Vimentin","SMA","CD31")

init <- c(0.01,0.01,0.01,0.01,1)
ParamsDirichlet <- matrix(nrow=length(MARKERS)*length(CELLTYPES),ncol = NB_TMENs+3)
colnames(ParamsDirichlet) <- c("cell_type","marker","gamma1","gamma2","gamma3","gamma4","km")
idx <- 1
for (i in 1:length(CELLTYPES)){
  for(j in 1:length(MARKERS)){
    #print(CELLTYPES[i])
    ParamsDirichlet[idx,c("cell_type","marker")] <- c(CELLTYPES[i],MARKERS[j])
    x_cm <- get_tmens_coord_marker(CELLTYPES[i],MARKERS[j])
    NCELLS <- nrow(x_cm) ## DIVIDE SS by nb of cells ?? If grad desc is too long
    estim_params <- optim(par= init, fn=func,data = x_cm,method = "L-BFGS-B",lower = c(-100,-100,-100,-100,0),upper=c(100,100,100,100,Inf))
    ParamsDirichlet[idx,c("gamma1","gamma2","gamma3","gamma4","km")]<- estim_params$par
    idx <- idx+1
  }
}
...
cMf2<- correlations_tmens_CM(MarkersCellsTMENs=MarkersCellsTMENS2,cellTypes=CELLTYPES, markers=FuncMarkers)
write_csv(cMf2%>%as_tibble(rownames=NA)%>%rownames_to_column(var="cell_marker"), "./CM_corr_tmensREDUCED_thresh0.3.csv")
# if(file.exists()){
#   
# }else{
#   cMf <- correlations_tmens_CM(MarkersCellsTMENs=MarkersCellsTMENS,cellTypes=CELLTYPES, markers=FuncMarkers)
...
############ ORDER BY MARKERS: phenotype --> environment

#pheatmap(cMf)
# rownames(cMf)<- unname(unlist(sapply(rownames(cMf),function(x){paste0(strsplit(x,";")[[1]][2],";",strsplit(x,";")[[1]][1])})))#sub('(\\w|\\.|\\_|\\ / |+);(\\w|.|_| / |+)', '\\2;\\1', rownames(cMf))#sub('([^;]);([^;])', '\\2;\\1^', rownames(cMf))
# 
 # cMf_ordered <- cMf%>%as_tibble(rownames=NA)%>%rownames_to_column(var="names")%>%separate(col="names",into=c("marker","cell_type"),sep=";")%>%group_by(marker)%>%arrange(marker)%>%mutate(names=paste0(marker,";",cell_type))%>%column_to_rownames(var="names")%>%dplyr::select(-c(cell_type,marker))
 # pdf("./figs/cM_orderMarkers.pdf",height=10,width=15)
 # pheatmap(as.matrix(cMf_ordered )%>%t,cluster_rows = FALSE,cluster_cols = FALSE)
 # dev.off()
...
```{r tableCorr, eval=FALSE,echo=TRUE}
...
tabTMENs <- as_tibble(cMf,rownames=NA)%>%
  ...
  mutate(region = set_tmens_names(region))%>%
  group_by(region)%>%
  filter(corr_val ==  max(corr_val))%>% #(corr_val%in% head(sort(corr_val,3)))
  dplyr::select(-(corr_val))%>%#mutate(sign  = "+")
  relocate(region)%>%arrange(region)

## Display table and save pdf

g <- tableGrob(tabTMENs,rows=NULL)
grid.newpage()
grid.draw(g)
#ggsave("./figs/tab_tmens1.pdf",g,width=4,height=3)

```


```{r tabCorrs2, eval=FALSE, echo=TRUE}
library(grid)
library(gridExtra)
...
tabTMENs2 <- as_tibble(cMf,rownames=NA)%>%
  ...
  group_by(region)%>%
  mutate(region = set_tmens_names(region))%>%
  filter(corr_val%in% head(sort(corr_val,decreasing=TRUE),3)& corr_val >=0.3)%>% #
  mutate(cell_phenotype = paste(marker,cell_type,collapse="\n"),.keep="unused")%>%
  dplyr::select(-(corr_val))%>%#mutate(sign  = "+")
  distinct(region, .keep_all = TRUE)%>%arrange(region)
tabTMENs2
...
g1 <- tableGrob(tabTMENs2,rows=NULL,theme = th1)
                #gpar.corefill = gpar(fill = "white", col = "grey95"))
grid.newpage()
grid.draw(g1)
#ggsave("./figs/tab_tmens2.pdf",g1,width=6,height=8)
...
```{r tabCorrsBio,fig.height=10, echo=TRUE}
library(grid)
library(gridExtra)
...
CM_TMENs_ct <- as_tibble(cMf2,rownames=NA)%>%
     rownames_to_column(var="names")%>%
...
pdf("./figs/cM_byCells2.pdf",height=8, width=ifelse(figWidth<6, 6, figWidth+2))
pheatmap(t(cMf2),cluster_cols = hclustCells,cluster_rows = FALSE,annotation_col=cellTypes,labels_col = Markers)#
dev.off()

####### HEATMAP ORGANIZED BY MARKERS
CM_TMENs_ph <- as_tibble(cMf2,rownames=NA)%>%
     rownames_to_column(var="names")%>%
     separate(names,into=c("cell_type","marker"),sep=";")%>%#mutate(marker=paste0(marker,"+"))%>%
     pivot_longer(cols=c('a1','a2','a3',"a4","a1a2","a1a3","a1a4","a2a3","a2a4","a3a4"),names_to = "region",values_to="corr_val")%>%
...
#%>%filter(marker!="H3K9ac_H3K27me3")

# i=0
# for (p in list_heatmapsCT){
#   i<- i+1
#   pdf(paste0("./figs/",strsplit(rownames(x)[1],';')[[1]][1],".pdf"),width=4);p;dev.off()
# }


# map(ct_df,.f = function(x) pheatmap(as.matrix(x%>%mutate(names= paste(cell_type,marker),.keep="unused")%>%column_to_rownames(var="names")),cluster_rows = TRUE,cluster_cols = FALSE))
# #pheatmap(as.matrix(ct_df%>%mutate(names= paste(cell_type,marker),.keep="unused")%>%column_to_rownames(var="names")),cluster_rows = TRUE,cluster_cols = FALSE)
# 
# pheatmap(as.matrix(ct_df%>%mutate(names= paste(cell_type,marker),.keep="unused")%>%column_to_rownames(var="names")))
# rm(list_heatmapsCT)
# pdf("./figs/corMatrix_ct.pdf",height=8,width=ifelse(figWidth<6, 6, figWidth+2)); pheatmap(t(as.matrix(cMf)),annotation_col =data.frame (cell_type =factor(pull(as_tibble(cMf,rownames=NA)%>%rownames_to_column(var="names")%>%separate(names,into=c("cell_type","marker"),sep=";"),cell_type), levels = cts,labels = CELLTYPES) )); dev.off()
...
##### CELL TYPE  hclust
# annot =data.frame(cell_type =factor(pull(as_tibble(cMf,rownames=NA)%>%rownames_to_column(var="names")%>%separate(names,into=c("cell_type","marker"),sep=";"),cell_type), levels = cts,labels = CELLTYPES),marker = factor(pull(as_tibble(cMf,rownames=NA)%>%rownames_to_column(var="names")%>%separate(names,into=c("cell_type","marker"),sep=";"),marker),levels = markersCorr,labels = markersCorr)) 
# 
# rownames(annot) <- colnames(t(cMf))
# pheatmap(t(as.matrix(cMf)),annotation_col =annot,cluster_cols = FALSE,cluster_rows=FALSE,show_colnames = FALSE)
...
library(RColorBrewer)
library(ggplot2)
CM_TMENs.pca <- dudi.pca(cMf2[,c('a1','a2','a3',"a4","a1a2","a1a3","a1a4","a2a3","a2a4","a3a4")],center=FALSE, scale=TRUE,scannf=FALSE,nf=3)
...
CM <- cMf2%>%as_tibble(rownames=NA)%>%rownames_to_column(var="names")%>%separate(col=names, into=c("cell_type","marker"),sep=";")%>%dplyr::select(c(cell_type,marker))
